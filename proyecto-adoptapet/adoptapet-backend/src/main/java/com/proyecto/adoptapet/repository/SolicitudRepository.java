...
public interface SolicitudRepository extends JpaRepository<Solicitud, Integer>{
	@Query("SELECT s FROM Solicitud s JOIN FETCH s.adoptante JOIN FETCH s.mascota LEFT JOIN FETCH s.programacionEntrega LEFT JOIN FETCH s.trabajador ORDER BY s.id_solicitud")
	List<Solicitud> listarSolicitudes();

	@Query("SELECT s FROM Solicitud s JOIN FETCH s.adoptante JOIN FETCH s.mascota LEFT JOIN FETCH s.programacionEntrega LEFT JOIN FETCH s.trabajador WHERE s.estado_solicitud = :estado ORDER BY s.id_solicitud")
	List<Solicitud> listarSolicitudesPorEstado(@Param("estado") String estado);

	@Query("SELECT s FROM Solicitud s JOIN FETCH s.adoptante JOIN FETCH s.mascota LEFT JOIN FETCH s.programacionEntrega LEFT JOIN FETCH s.trabajador WHERE s.id_solicitud = :idSolicitud")
